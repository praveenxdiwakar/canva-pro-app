...
export default function Redeem() {
  ...
  const executeRedemption = async () => {
    ...
    try {
      ...
      // Log Redemption in History FIRST to ensure database accepts it
      ...
      // IF DATABASE FAILS (e.g. missing table), THROW ERROR AND STOP
      if (insertError) {
        console.error("DB Insert Error:", insertError);
        throw new Error("Failed to save redemption to cloud. Please try again.");
      }

      // If DB insert succeeded, deduct points
